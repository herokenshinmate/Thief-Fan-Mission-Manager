...
On first launch (or from the **Settings** menu any time), set for each game
you own:
...
Click the green **Play** button on a mission to launch it directly — no
need to pick it again from the loader's own list. If no executable is
configured for that mission's game yet, you'll be prompted to set one in
Settings.
...
- **Properties** — edit every field, including notes, tags, and a manual
  Thief Guild URL if the automatic lookup didn't find the right page.
...
release year, tags, community rating, description, and series info.
Already-linked missions are re-checked once automatically at startup.
